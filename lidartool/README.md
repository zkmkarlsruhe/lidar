...
The lidartool is intended to be used by artists for interactive applications or museums to gather information about visitor flow using laser range detection.
...
* processing object data via Lua scripts or calling shell scripts at certain events 

An integrated web server provides a web browser-based GUI and a REST API.
...
If you want to install on a **Raspberry Pi 3 or 4**, follow these [instructions](doc/RaspberryPi/README.md).
...
See [doc/gui](doc/gui/README.md) for description of the Web GUI.
...
See section [Device Name Management](#device-name-management) for convenience handling of device names and with assigning devices by serial number to device names.
...
Tracked objects, e.g. humans, get assigned a simple id and a uuid which is guaranteed to never be used twice. Once detected, objects are tracked over time and their data can be processed in several ways, e.g. send to a host via OSC or as JSON formatted messages via a websocket. Objects are only detected if tracking is enabled.
...
Tracking operates with a given frame rate independent of the frame rates of the connected devices. If a sensor delivers a lower frame rate then the system frame rate, the tracking data will be smoothed. The default system frame rate is 60Hz.
...
Regions are rectangular or elliptical areas on which specific processing of objects can happen. Regions are managed via the command line and the Web UI. Regions can be created and deleted and parameters can be assigned. Their geometry can be also be edited via the graphical web interface.
...
A universal way of processing and sending data via different communication channels - like file logging, OSC, MQTT, shell / Lua scripting or websocket - are **Observers**. Multiple observers allow for processing data in parallel.
...
| Manufacturer                                                                              | Model                                                                                                   | DevType | Range   | SmpFq    | ScnFq  | Res    | LasPow | WavLen | Notes                                                                    |
|:----------------------------------------------------------------------------------------- |:------------------------------------------------------------------------------------------------------- |:------- |:------- |:-------- |:------ |:------ |:------ |:------ |:------------------------------------------------------------------------ |
...
| [YDLidar](https://www.ydlidar.com/lidars.html)                                            | Most devices with [Serial Interface](https://github.com/YDLIDAR/YDLidar-SDK/blob/master/doc/Dataset.md), see [Supported Device Types](#supported-device-types) | ydlidar | *       | *        | *      | *      | *      | *      | *                                                                        |
| [LDRobot](https://www.ldrobot.com/SensorIndex?twoIndex=0&threeIndex=0)                                      | [LD06/ LD19](https://www.inno-maker.com/wp-content/uploads/2020/11/LDROBOT_LD06_Datasheet.pdf)          | ld06    | 12 m    | 4.5 kHz  | 10 Hz  | 1°     | 25 mW  | 905 nm | Waterproof and dustproof-IPX4, Temperature: -10/40°C, 30000 lux, 10000 h |
| [LDRobot](https://www.ldrobot.com/SensorIndex?twoIndex=0&threeIndex=0)                                      | [STL27L](https://github.com/May-DFRobot/DFRobot/blob/master/SEN0589_Datasheet.pdf)                      | st27    | 25m     | 21.6 kHz | 10 Hz  | 0.167° | -      | 905 nm | Waterproof and dustproof-IPX5, Temperature: -10/50°C, 60000 lux, 10000 h |
